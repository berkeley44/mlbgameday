...
payload.gd_inning_all <- function(urlz, ...) {
    ...
    bats <- atbat %>% dplyr::select(start_pos2_person_id, num, inning, inning_side) %>% dplyr::rename(pos2_person_id = start_pos2_person_id)
    ...
    action <- dplyr::left_join(action, events, by = c("pos2_person_id", "inning", "inning_side", "des"))
